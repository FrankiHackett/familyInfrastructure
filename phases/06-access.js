...
export async function runAccess(cfg, inputs, vercelDeploymentUrl, iface) {
  ...
  logger.info(`  The domain may show a yellow "No deployment" label — this is fine at this stage,`)
  logger.info(`  it just means no code has been deployed yet. As long as the domain is listed,`)
  logger.info(`  Vercel has registered it and you can proceed.`)
  logger.info(`  (A green SSL tick will appear later once a deployment exists.)`)
  await confirm(`Press y once ${subdomain} is listed in Vercel Domains (even with "No deployment")`, iface)
