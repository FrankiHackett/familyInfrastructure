...
/**
 * Scan source files under appDir for env var references (import.meta.env.* and process.env.*).
 * Returns a Set of variable names found.
 */
function scanEnvVarReferences(appDir) {
  const refs = new Set()
  const pattern = /(?:import\.meta\.env|process\.env)\.([A-Z_][A-Z0-9_]*)/g
  const skip = new Set(['node_modules', 'dist', 'build', '.git', 'coverage', 'playwright-report'])
...
/**
 * Scan the app source for env var references, compare against what's already covered,
 * and prompt for any that are missing. Also offers a free-form entry for anything
 * the scan may have missed. Saves results to cfg.app_env_vars[appName].
 *
 * coveredKeys: array of var names already being injected by the standard build
 */
export async function collectAppEnvVars(cfg, appName, appDir, coveredKeys, iface) {
  const existing  = cfg.app_env_vars?.[appName] ?? {}

  // Auto-populate known keys derivable from top-level config so we don't prompt for them
  const autoFilled = {}
  if (cfg.anthropic?.api_key && !existing['VITE_ANTHROPIC_API_KEY']) {
    autoFilled['VITE_ANTHROPIC_API_KEY'] = cfg.anthropic.api_key
  }
  if (cfg.supabase?.project_url && !existing['SUPABASE_URL']) {
    autoFilled['SUPABASE_URL'] = cfg.supabase.project_url
  }
  if (cfg.supabase?.anon_key && !existing['SUPABASE_ANON_KEY']) {
    autoFilled['SUPABASE_ANON_KEY'] = cfg.supabase.anon_key
  }

  const covered   = new Set([...coveredKeys, ...Object.keys(existing), ...Object.keys(autoFilled)])
  const refs      = scanEnvVarReferences(appDir)
  const missing   = [...refs].filter(k => !covered.has(k)).sort()

  if (missing.length === 0 && Object.keys(existing).length > 0) return

  console.log('\n  ── App-specific Environment Variables ──────────────────────────')

  const vars = { ...autoFilled, ...existing }

  if (missing.length > 0) {
    console.log(`  Found ${missing.length} variable(s) referenced in the app code not yet covered:\n`)
    for (const key of missing) {
      let value
      while (!value) {
        value = (await ask(iface, `  ${key}=  `)).trim()
        if (!value) console.log('  → Value required (leave blank later to remove this var).')
      }
      vars[key] = value
    }
  } else {
    console.log('  All env vars referenced in app code are already covered.')
  }

  console.log('\n  Any additional variables not found by the scan? (KEY=value, blank to finish)\n')
  while (true) {
