...
export async function collectAppEnvVars(cfg, appName, iface) {
  ...
  console.log('  Enter any custom env vars to inject into this app\'s Vercel project.')
  console.log('  Format: KEY=value — leave blank to finish.\n')
...
export async function collectBootstrapInputs(cfg, iface) {
  ...
  // Collect app-specific env vars before the confirmation screen
  await collectAppEnvVars(cfg, inputs.appName, iface)

  // Confirm before proceeding
  const appVars = cfg.app_env_vars?.[inputs.appName]
  const appVarCount = appVars ? Object.keys(appVars).length : 0
  console.log('\n  ─────────────────────────────────────────────')
  ...
  if (appVarCount > 0) {
    console.log(`  App env vars: ${Object.keys(appVars).join(', ')}`)
  }
  console.log('  ─────────────────────────────────────────────')
