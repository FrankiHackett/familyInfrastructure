 ...
 */
export async function collectBootstrapInputs(cfg) {
  const iface = rl()
  const inputs = {}
...
/**
 * Open a file in $EDITOR and wait for the user to close it.
 */
export async function openInEditor(filePath) {
  const editor = process.env.EDITOR || process.env.VISUAL || 'nano'
  const { spawn: spawnRaw } = await import('node:child_process')
  return new Promise((resolve, reject) => {
    const child = spawnRaw(editor, [filePath], { stdio: 'inherit', shell: false })
